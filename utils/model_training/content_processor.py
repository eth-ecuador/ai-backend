...
class ContentProcessor:
    ...
    def process_page(self, url, html_content, max_content_length=10000):
      ...
      if element.find('article'):
        ...
        metadata = {
          'url': url,
          'type': type,
          'breadcrumbs': " | ".join(self._get_breadcrumbs(soup)),  # Convert list to string
          'headings': "; ".join([f"{h['level']}: {h['text']}" for h in self._get_headings(soup)]),  # Flatten headings
          'last_updated': self._get_last_modified(soup) or "unknown",
          'related_links': "; ".join([f"{link['text']}: {link['url']}" for link in self._get_related_links(soup)]),  # Flatten links
          'content': text
        }
    ...
    def _get_breadcrumbs(self, soup):
        ...
        """
        breadcrumb = soup.find('nav', class_='breadcrumb') or soup.find('ol', class_='breadcrumb') or soup.find('ul', class_='breadcrumb')
        if breadcrumb:
    ...
    def _get_last_modified(self, soup):
        ...
        """
        last_modified = soup.find('time') or soup.find('span', class_='last-modified')
    ...
    def _get_related_links(self, soup):
        ...
        """
        related_links = soup.find('div', class_='related-links') or soup.find('aside', class_='related')
        if related_links:
